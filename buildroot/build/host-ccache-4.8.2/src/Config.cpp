...
#  else
  return home_dir + "/.buildroot-ccache";
#  endif
}
...
Config::read(const std::vector<std::string>& cmdline_config_settings)
{
  ...
  const std::string legacy_ccache_dir = Util::make_path(home_dir, ".buildroot-ccache");
...
#else
  const char* const env_xdg_cache_home = nullptr;
  const char* const env_xdg_config_home = nullptr;
#endif
  ...
  if (env_ccache_configpath) {
    ...
    const char* const env_ccache_dir = getenv("BR_CACHE_DIR");
