...
# Append color settings and prompt to root's .bashrc
...
# Append the same settings to the default user's .bashrc (for non-root users)
mkdir -p "${TARGET_DIR}/etc/skel"
cat << 'EOF' >> "${TARGET_DIR}/etc/skel/.bashrc"
# Color definitions
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[0;33m'
...
# Copy the default user's .bashrc to the root directory for consistency
cp "${TARGET_DIR}/etc/skel/.bashrc" "${TARGET_DIR}/root/.bashrc"
